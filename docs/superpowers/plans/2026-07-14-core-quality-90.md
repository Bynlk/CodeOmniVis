...
Run: `pnpm build && pnpm --filter @codeomnivis/mcp test && pnpm --filter @bynlk/codeomnivis test && pnpm --filter @bynlk/codeomnivis test -- integration/demoCrossLayer.test.ts`

Expected: PASS; MCP stdio handshake still works and the intentionally non-runnable demo fixture still produces its expected cross-layer graph.
