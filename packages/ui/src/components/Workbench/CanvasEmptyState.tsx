...
const EMPTY_COPY: Record<Exclude<WorkbenchView, 'quality'>, { titleKey: string; title: string; descriptionKey: string; description: string }> = {
  architecture: {
    ...
    description: 'Run analysis again or open Full graph to inspect every parsed implementation node.',
  },
  requests: {
    ...
    description: 'Request flow appears after render, API call, handler, service, or query relationships are found.',
  },
  data: {
...
export function CanvasEmptyState({ view, hasSearchQuery, isAnalyzed = true }: CanvasEmptyStateProps) {
  ...
  const copy = !isAnalyzed
    ? {
        ...
        description: 'Run the first analysis to populate this workspace with architecture nodes and relationships.',
      }
    : hasSearchQuery
    ? {
  ...
  return (
    <div className="flex h-full items-center justify-center p-8 text-center">
      <div className="max-w-sm">
        <div className="mx-auto mb-4 h-8 w-8 rounded-md border border-border-strong bg-surface-panel" aria-hidden="true" />
        <h2 className="text-sm font-semibold text-content">{t(copy.titleKey, copy.title)}</h2>
        <p className="mt-2 text-xs leading-5 text-content-muted">{t(copy.descriptionKey, copy.description)}</p>
      </div>
    </div>
  )
}
