...
export function TestExplorer({ graph, selectedNodeId, onNodeSelect }: TestExplorerProps) {
  ...
  const frameworks = useMemo(() => [...new Set(allGroups.flatMap(group => {
    const value = group.suite.metadata as { framework?: TestFramework }
    return value.framework ? [value.framework] : []
  }))].sort(), [allGroups])
  return (
    <section data-testid="test-explorer" className="h-full overflow-auto bg-[#0b0e13] p-3" aria-label={t('workbench.tests.explorer', 'Test explorer')}>
      ...
      <div className="mb-3 grid grid-cols-2 gap-2">
        <label className="text-[10px] text-content-muted">
          ...
          <select value={framework} onChange={event => setFramework(event.target.value as TestFramework | 'all')} className="w-full rounded border border-border-subtle bg-surface px-2 py-1 text-[11px] text-content">
            <option value="all">{t('workbench.tests.all', 'All')}</option>
            {frameworks.map(value => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <label className="text-[10px] text-content-muted">
          <span className="mb-1 block">{t('workbench.tests.status', 'Status')}</span>
          <select value={status} onChange={event => setStatus(event.target.value as typeof status)} className="w-full rounded border border-border-subtle bg-surface px-2 py-1 text-[11px] text-content">
            <option value="all">{t('workbench.tests.all', 'All')}</option>
          ...
          <div className="border-t border-border-subtle p-1">
            ...
            {group.fixtures.length > 0 ? (
              <div className="mt-1 border-t border-border-subtle px-2 py-1.5 text-[10px] text-content-muted">
                {t('workbench.tests.fixtures', 'Fixtures')}: {group.fixtures.map(fixture => fixture.name).join(', ')}
              </div>
            ) : null}
          </div>
        </details>
