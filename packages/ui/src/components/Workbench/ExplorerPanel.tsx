...
const VIEW_META: Record<WorkbenchView, { titleKey: string; title: string; descriptionKey: string; description: string }> = {
...
export function ExplorerPanel({ graph, view, isAnalyzed = true, selectedNodeId, onNodeSelect }: ExplorerPanelProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="border-b border-border-subtle px-3 py-3">
        ...
        <p className="mt-1 pl-6 text-[11px] text-content-muted">{t(meta.descriptionKey, meta.description)}</p>
      </div>
      <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2 text-[11px] text-content-muted">
        <span>{t('workbench.explorer', 'Explorer')}</span><span className="font-mono tabular-nums">{graph?.nodes.length ?? 0}</span>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto py-2">
        {groups.length === 0 ? (
          <div className="px-4 py-8 text-center text-[11px] leading-5 text-content-muted">
            {!isAnalyzed
              ? t('workbench.empty.notAnalyzed.explorer', 'Run the first analysis to populate this workspace.')
              : view === 'quality'
          ...
          <section key={type} className="mb-3">
            <div className="flex items-center gap-2 px-3 py-1 text-[10px] font-medium text-content-muted">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: NODE_COLORS[type] }} />
              <span className="min-w-0 flex-1 truncate">{t(`nodeType.${type}`, type.replaceAll('_', ' '))}</span>
            ...
            <div>
              {nodes.map(node => (
                ...
                >
                  <WorkbenchIcon name={node.type === 'db_model' ? 'database' : 'file'} className={`h-3.5 w-3.5 shrink-0 ${selectedNodeId === node.id ? 'text-primary-400' : 'text-content-muted'}`} />
