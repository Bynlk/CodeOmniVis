...
const suite = {
  id: 'suite', type: 'test_suite', name: 'suite', filePath: 'a.test.ts', line: 1, column: 1,
  metadata: { framework: 'vitest', kind: 'describe' },
}
const testCase = {
  id: 'case', type: 'test_case', name: 'suite > works', filePath: 'a.test.ts', line: 2, column: 1,
  metadata: { framework: 'vitest', isParameterized: false, disabled: false },
}
const fixture = {
  id: 'fixture', type: 'test_fixture', name: 'beforeEach', filePath: 'a.test.ts', line: 1, column: 1,
  metadata: { framework: 'vitest', lifecycle: 'before_each' },
}
const covers = {
  id: 'covers', source: 'case', target: 'service', type: 'covers', confidence: 'certain',
...
describe('tests service', () => {
  it('validates and returns the shared test projection', async () => {
    respond({ data: {
      suites: [suite], cases: [testCase], fixtures: [fixture], coverage: [covers],
      summary: {
        suites: 1, cases: 1, fixtures: 1, coveredTargets: 1, uncoveredTargets: 2,
  ...
  it('uses safe zero defaults for malformed summary counters', async () => {
    respond({ data: {
      suites: [], cases: [], fixtures: [], coverage: [],
      summary: { suites: '1', cases: null, fixtures: {}, byFramework: [] },
    } })

    await expect(getTests()).resolves.toMatchObject({
      summary: {
        suites: 0, cases: 0, fixtures: 0, coveredTargets: 0, uncoveredTargets: 0,
        byFramework: {},
      },
    })
  })
  ...
  it.each([
    ...
    { data: { summary: {}, suites: [{ ...suite, type: 'unknown' }], cases: [], fixtures: [], coverage: [] } },
    { data: { summary: {}, suites: [], cases: [{ ...testCase, line: '2' }], fixtures: [], coverage: [] } },
    { data: { summary: {}, suites: [], cases: [], fixtures: [], coverage: [{ ...covers, confidence: 'guess' }] } },
