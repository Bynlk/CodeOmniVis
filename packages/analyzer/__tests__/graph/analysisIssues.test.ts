...
describe('detectAnalysisIssues', () => {
  it('deduplicates and sorts results from independent detectors', () => {
    vi.spyOn(ConsistencyChecker.prototype, 'check').mockReturnValue({
      issues: [issue('z'), issue('a')],
      summary: { total: 2, critical: 0, warning: 0, info: 2 },
    })
    ...
    expect(
      detectAnalysisIssues({ nodes: [], edges: [] }, '/project', []).map((item) => item.id),
    ).toEqual(['a', 'z'])
  })

  it('degrades one detector failure without suppressing the others', () => {
    const errors: SerializableParseError[] = []
    vi.spyOn(ConsistencyChecker.prototype, 'check').mockImplementation(() => {
    ...
    expect(
      detectAnalysisIssues({ nodes: [], edges: [] }, '/project', errors).map((item) => item.id),
    ).toEqual(['kept'])
