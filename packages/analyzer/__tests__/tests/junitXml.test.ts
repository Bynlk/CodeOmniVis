...
const snapshot: Pick<ProjectSnapshot, 'graph'> = {
  graph: { nodes: [{
    id: 'test_case:CheckoutTest:accepts valid card', type: 'test_case',
    name: 'CheckoutTest > accepts valid card', filePath: 'CheckoutTest.kt', line: 1, column: 1,
...
describe('JUnit XML import', () => {
  ...
  it('rejects XML entity declarations before parsing', () => {
    expect(() => importJunitXml(path.join(results, 'entity.xml'), snapshot)).toThrow('entity declarations')
  })
})
