...
const context: TestDiscoveryContext = {
  projectRoot: '/fixture',
  projectMeta: {
    root: '/fixture', frontendFramework: 'unknown', backendFramework: 'unknown',
...
describe('test adapter registry', () => {
  it('isolates adapter failures and continues in deterministic order', async () => {
    const adapters: TestAdapter[] = [
      { name: 'declines', canHandle: () => false, discover: async () => ({ nodes: [], edges: [], errors: [] }) },
      { name: 'broken', canHandle: () => true, discover: async () => { throw new Error('broken adapter') } },
      {
        name: 'working',
        canHandle: () => true,
        discover: async () => ({
          nodes: [{
            id: 'test_suite:test.ts:suite', type: 'test_suite', name: 'suite', filePath: 'test.ts',
            line: 1, column: 1, metadata: { framework: 'vitest', kind: 'file' },
          }],
          edges: [], errors: [],
        }),
      },
    ...
    expect(result.errors).toEqual([
      expect.objectContaining({ file: 'test.ts', severity: 'warning', message: expect.stringContaining('broken') }),
    ])
  })
})
