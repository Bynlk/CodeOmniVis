...
describe('CypressAdapter', () => {
    ...
    expect(result.nodes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: 'test_suite',
          metadata: expect.objectContaining({ framework: 'cypress' }),
        }),
  ...
  it('rejects unreadable candidates and degrades discovery to a warning', async () => {
    const missingFile = 'cypress/missing.fixture.ts'

    expect(CypressAdapter.canHandle(missingFile, context)).toBe(false)
    const result = await CypressAdapter.discover(missingFile, context)
    expect(result.nodes).toEqual([])
    expect(result.edges).toEqual([])
    expect(result.errors).toEqual([
      expect.objectContaining({
        file: missingFile,
        message: expect.stringContaining('Cypress discovery failed'),
        severity: 'warning',
      }),
    ])
  })
