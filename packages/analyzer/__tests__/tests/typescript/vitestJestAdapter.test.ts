    ...
    expect(cases).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          name: 'checkout > cards > rejects expired card',
          metadata: expect.objectContaining({ disabled: true }),
        }),
        expect.objectContaining({
          name: 'checkout > cards > accepts %s',
          metadata: expect.objectContaining({ isParameterized: true }),
        }),
      ]),
    )
    expect(
      result.nodes.some(
        (node) => node.type === 'test_fixture' && node.metadata.lifecycle === 'before_each',
      ),
    ).toBe(true)
  ...
  it('selects Jest from import evidence', async () => {
    ...
    expect(result.nodes).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: 'test_case',
          metadata: expect.objectContaining({ framework: 'jest' }),
        }),
        expect.objectContaining({
          type: 'test_fixture',
          metadata: expect.objectContaining({ lifecycle: 'after_all' }),
        }),
      ]),
    )
  })
