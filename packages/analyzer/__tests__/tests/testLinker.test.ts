...
describe('testLinker', () => {
  it('links a directly called import with certain evidence and no dangling endpoints', () => {
    const production: OmniNode = {
      id: 'service:src/checkout.ts:checkout',
      type: 'service',
      name: 'checkout',
      filePath: 'src/checkout.ts',
      line: 1,
      column: 1,
      metadata: { className: null, methodName: 'checkout' },
    }
    const testCase: OmniNode = {
      id: 'test_case:tests/checkout.test.ts:checkout',
      type: 'test_case',
      name: 'checkout',
    ...
    const result = linkTestsToProduction(
      discovery,
      [production],
      '/fixture',
      new Map([
    ...
    expect(
      result.edges.every(
        (edge) =>
          [testCase.id, production.id].includes(edge.source) &&
          [testCase.id, production.id].includes(edge.target),
      ),
    ).toBe(true)
  })
  ...
  it('reads source from disk, supports aliases, and links exact route references', () => {
    ...
    const production: OmniNode[] = [
      ...
      },
      {
        ...
        line: 1,
        column: 1,
        metadata: { method: 'GET', route: '/api/orders', isNextApiRoute: true },
      },
    ]
    const testCase: OmniNode = {
      id: 'case',
    ...
    const existing = {
      ...
      metadata: { evidence: 'direct_import' as const },
    }

    try {
      const result = linkTestsToProduction(
        { nodes: [testCase], edges: [existing], errors: [] },
        production,
        root,
      )
      expect(result.edges).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({
            target: 'route',
            confidence: 'inferred',
            metadata: { evidence: 'route_reference' },
          }),
        ]),
      )
    } finally {
  ...
  it('ignores unreadable sources and ambiguous imports', () => {
    ...
    const duplicate: OmniNode[] = [
      {
        ...
        line: 1,
        column: 1,
        metadata: { className: null, methodName: 'run' },
      },
      {
        id: 'b',
        type: 'service',
    ...
    expect(
      linkTestsToProduction(
        discovery,
        duplicate,
        '/missing',
        new Map([['missing.test.ts', "import { run } from './src/run'\nrun()"]]),
      ).edges,
    ).toEqual([])
  })
})
