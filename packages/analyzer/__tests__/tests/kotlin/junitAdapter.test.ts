...
describe('JunitAdapter', () => {
  it('discovers JUnit 5 classes, lifecycle and parameter source', async () => {
    ...
    expect(result.nodes).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ type: 'test_case', name: expect.stringContaining('rejectsExpiredCard'), metadata: expect.objectContaining({ isParameterized: true, parameterSource: 'expiredCards' }) }),
