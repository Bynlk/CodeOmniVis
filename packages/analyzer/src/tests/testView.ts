...
function isTestNode(node: OmniNode): node is Extract<OmniNode, { type: 'test_suite' | 'test_case' | 'test_fixture' }> {
...
export function projectTestView(graph: OmniGraph, filters: TestViewFilters = {}): TestView {
  const testNodes = graph.nodes.filter(isTestNode).filter(node =>
    !filters.framework || node.metadata.framework === filters.framework,
  )
  const testIds = new Set(testNodes.map(node => node.id))
  const targetMatches = (node: OmniNode): boolean => !filters.target
  ...
  const coverage = graph.edges.filter(edge =>
    edge.type === 'covers' && testIds.has(edge.source) && (!filters.target || targetIds.has(edge.target)),
  )
  const visibleCaseIds = filters.target ? new Set(coverage.map(edge => edge.source)) : null
  const cases = testNodes.filter(node => node.type === 'test_case' && (!visibleCaseIds || visibleCaseIds.has(node.id)))
  ...
  const productionTargets = graph.nodes.filter(node =>
    node.type !== 'test_suite' && node.type !== 'test_case' && node.type !== 'test_fixture' && targetMatches(node),
  )
  const byFramework: Record<TestFramework, number> = {
    vitest: 0, jest: 0, playwright: 0, cypress: 0, junit4: 0, junit5: 0, kotest: 0,
  }
