...
import { isJsonObject, type ProjectSnapshot, type TestRunCaseResult, type TestRunImport } from '@codeomnivis/shared'
...
function caseResult(suite: string, value: unknown): TestRunCaseResult | null {
  ...
  const failureMessage = isJsonObject(failure)
    ...
    : stringValue(failure)
  const status = value.skipped !== undefined ? 'skipped' : value.failure !== undefined || value.error !== undefined ? 'failed' : 'passed'
  return {
...
export function importJunitXml(
  ...
  if (/<!DOCTYPE|<!ENTITY/iu.test(xml)) throw new Error('JUnit XML entity declarations are not allowed')
  ...
  const unmatched = cases.filter(result => !staticCases.some(node =>
