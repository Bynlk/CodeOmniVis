...
function ancestors(call: CallExpression): string[] {
  ...
  for (const ancestor of [...call.getAncestors()].reverse()) {
    if (!Node.isCallExpression(ancestor) || callPath(ancestor).join('.') !== 'test.describe')
      continue
...
export const PlaywrightAdapter: TestAdapter = {
  ...
  canHandle(filePath, context) {
    try {
      return /@playwright\/test/u.test(
        fs.readFileSync(path.resolve(context.projectRoot, filePath), 'utf8'),
      )
    } catch {
      return false
    }
    ...
    try {
      ...
      for (const call of source.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        ...
        if (parts.join('.') === 'test.describe' && name) {
          ...
          const node: OmniNode = {
            id: testNodeId('test_suite', filePath, qualified),
            type: 'test_suite',
            name: qualified,
            filePath,
            ...point,
            metadata: { framework: 'playwright', kind: 'describe' },
          }
        ...
        if (parts.includes('extend')) {
          ...
          if (object && Node.isObjectLiteralExpression(object)) {
            for (const property of object.getProperties()) {
              ...
              const node: OmniNode = {
                id: testNodeId('test_fixture', filePath, fixtureName),
                type: 'test_fixture',
                name: fixtureName,
                filePath,
                ...source.getLineAndColumnAtPos(property.getStart()),
                metadata: { framework: 'playwright', lifecycle: 'factory' },
              }
        ...
        if (
          parts[0] !== 'test' ||
          parts.includes('describe') ||
          parts.some((part) => part.startsWith('before') || part.startsWith('after')) ||
          !name
        )
          continue
        const qualified = qualifiedTestName(scope, name)
        const node: OmniNode = {
          id: testNodeId('test_case', filePath, qualified),
          type: 'test_case',
          name: qualified,
          filePath,
          ...point,
        ...
        for (const fixtureName of callbackFixtureNames(call)) {
          ...
          if (fixture)
            edges.push({
              id: createEdgeId(node.id, 'uses_fixture', fixture.id),
              source: node.id,
              target: fixture.id,
              type: 'uses_fixture',
              confidence: 'certain',
              metadata: { usage: 'parameter' },
            })
        }
      }
      ...
      return {
        ...
        errors: [
          {
            file: filePath,
            message: `Playwright discovery failed: ${error instanceof Error ? error.message : String(error)}`,
            severity: 'warning',
          },
        ],
      }
