...
function suiteNames(call: CallExpression): string[] {
  ...
  for (const ancestor of [...call.getAncestors()].reverse()) {
    if (!Node.isCallExpression(ancestor) || !['describe', 'context'].includes(callPath(ancestor)[0])) continue
...
export const CypressAdapter: TestAdapter = {
  ...
  canHandle(filePath, context) {
    try {
      ...
      return filePath.includes('cypress/e2e') || /\bcy\.(?:visit|request)\b/u.test(source) || /from\s+['"]cypress['"]/u.test(source)
    } catch {
      return false
    }
  },
  async discover(filePath, context) {
    try {
      ...
      if (!/\bcy\.|from\s+['"]cypress['"]/u.test(sourceText) && !filePath.includes('cypress/e2e')) return { nodes: [], edges: [], errors: [] }
      ...
      for (const call of source.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        ...
        if (['describe', 'context'].includes(base) && own) {
          ...
          const node: OmniNode = { id: testNodeId('test_suite', filePath, name), type: 'test_suite', name, filePath, ...point, metadata: { framework: 'cypress', kind: 'describe' } }
          suites.set(name, node)
          nodes.push(node)
        } else if (['before', 'beforeEach', 'afterEach', 'after'].includes(base)) {
          const lifecycle = base === 'before' ? 'before_all' : base === 'after' ? 'after_all' : base === 'beforeEach' ? 'before_each' : 'after_each'
          const name = qualifiedTestName(scope, `${base}@${point.line}`)
          nodes.push({ id: testNodeId('test_fixture', filePath, name), type: 'test_fixture', name, filePath, ...point, metadata: { framework: 'cypress', lifecycle } })
        } else if (['it', 'test'].includes(base) && own) {
          const name = qualifiedTestName(scope, own)
          const node: OmniNode = { id: testNodeId('test_case', filePath, name), type: 'test_case', name, filePath, ...point, metadata: { framework: 'cypress', isParameterized: false, disabled: callPath(call).includes('skip') } }
          nodes.push(node)
      ...
      return { nodes: [], edges: [], errors: [{ file: filePath, message: `Cypress discovery failed: ${error instanceof Error ? error.message : String(error)}`, severity: 'warning' }] }
    }
  },
}
