...
export const JunitAdapter: TestAdapter = {
  ...
  canHandle(filePath, context) {
    return (
      ...
      (context.projectMeta.backendFramework !== 'unknown' || filePath.includes('/test/'))
    )
  },
    ...
    try {
      ...
      for (const match of loaded.source.matchAll(/(?:@Nested\s+)?(?:inner\s+)?class\s+(\w+)/gu)) {
        ...
        const node: OmniNode = {
          ...
          name,
          filePath,
          ...loaded.position(match.index),
          metadata: { framework: selected, kind: suites.length === 0 ? 'class' : 'nested_class' },
        }
        suites.push(node)
        nodes.push(node)
      }
      ...
      for (const match of loaded.source.matchAll(declaration)) {
        ...
        const lifecycle =
          annotations.includes('@BeforeEach') || annotations.includes('@Before')
            ? 'before_each'
            : annotations.includes('@AfterEach') || annotations.includes('@After')
              ? 'after_each'
              : annotations.includes('@BeforeAll')
                ? 'before_all'
                : annotations.includes('@AfterAll')
                  ? 'after_all'
                  : null
        if (lifecycle) {
          nodes.push({
            id: testNodeId('test_fixture', filePath, functionName),
            type: 'test_fixture',
            name: functionName,
            filePath,
            ...point,
            metadata: { framework: selected, lifecycle },
          })
          continue
        }
        ...
        const node: OmniNode = {
          ...
          metadata: {
            framework: selected,
            isParameterized: annotations.includes('@ParameterizedTest'),
            ...(source ? { parameterSource: source } : {}),
            disabled: annotations.includes('@Disabled') || annotations.includes('@Ignore'),
          },
        }
        nodes.push(node)
        if (owner)
      ...
      return {
        ...
        errors: [
          {
            file: filePath,
            message: `JUnit discovery failed: ${error instanceof Error ? error.message : String(error)}`,
            severity: 'warning',
          },
        ],
      }
