    ...
    for (const match of source.matchAll(imports)) {
      ...
      for (const binding of match[1].split(',')) {
        ...
        const candidates = fileNodes.filter(
          (node) =>
            node.name === original ||
            ('methodName' in node.metadata && node.metadata.methodName === original),
        )
        if (candidates.length !== 1) continue
        const target = candidates[0]
        const called = new RegExp(
          `\\b${local.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}\\s*\\(`,
          'u',
        ).test(source)
        const edge: OmniEdge = {
          id: createEdgeId(testCase.id, 'covers', target.id),
          source: testCase.id,
          target: target.id,
          type: 'covers',
          confidence: called ? 'certain' : 'inferred',
          metadata: { evidence: called ? 'direct_call' : 'direct_import' },
        }
        const current = linked.get(edge.id)
        if (!current || (current.confidence === 'inferred' && edge.confidence === 'certain'))
          linked.set(edge.id, edge)
      }
    }
