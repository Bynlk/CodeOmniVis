...
function detectBackend(dependencies: Record<string, string>): FrameworkType {
  ...
  if (dependencies['@trpc/server'] || Object.keys(dependencies).some(name => name.endsWith('/trpc'))) {
    return 'trpc'
  }
...
function findPrismaSchema(root: string): string | null {
  ...
  return fg.sync('packages/*/{prisma/,}schema.prisma', {
    cwd: root,
    onlyFiles: true,
    followSymbolicLinks: false,
  }).map(normalizePath).sort()[0] ?? null
}

function detectDatabase(root: string, dependencies: Record<string, string>): DatabaseType {
  if (findPrismaSchema(root) || dependencies.prisma || dependencies['@prisma/client']) return 'prisma'
...
function findTsrpcPaths(root: string): {
  ...
  const files = fg.sync([
    '{src/,}{api,server/api,protocols,shared/protocols}/**/*.{ts,tsx}',
  ], {
    cwd: root,
    onlyFiles: true,
    followSymbolicLinks: false,
...
function findTypeormDirs(root: string): string[] {
  return existingDirectories(root, [
    'entity', 'entities', 'src/entity', 'src/entities',
    'model', 'models', 'src/model', 'src/models',
...
export async function detectProject(
  ...
  for (const child of ['frontend', 'backend', 'apps/web', 'app']) {
    mergeDependencies(
      dependencies,
      readManifest(path.join(root, child, 'package.json'), onWarning),
    )
  }
  const packages = discoverWorkspacePackages(root)
  for (const pkg of packages) {
    for (const dependency of [...pkg.dependencies, ...pkg.devDependencies]) dependencies[dependency] = '*'
  }
  ...
  const workspaceFrontendDirs = packages
    .filter(pkg => [...pkg.dependencies, ...pkg.devDependencies].some(dep => dep === 'next' || dep === 'react'))
    .map(pkg => `${pkg.path}/src`)
  const workspaceBackendDirs = packages
    .filter(pkg => [...pkg.dependencies, ...pkg.devDependencies].some(dep => (
  ...
  return applyProjectConfig({
    ...
    tsConfigPath: (() => {
      const relative = firstExisting(root, ['tsconfig.json', 'apps/web/tsconfig.json', 'app/tsconfig.json', 'src/tsconfig.json'])
