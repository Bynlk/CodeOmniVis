...
export class OmniDatabase implements AnalysisStore {
  ...
  getNodesByType(type: NodeType): OmniNode[] {
    return this.attempt(`get nodes by type ${type}`, [], (repositories) =>
      repositories.nodes.byType(type),
    )
  }

  getNodesByFile(filePath: string): OmniNode[] {
    return this.attempt(`get nodes by file ${filePath}`, [], (repositories) =>
      repositories.nodes.byFile(filePath),
    )
  }
  ...
  getAllEdges(): OmniEdge[] {
    return this.attempt('get all edges', [], (repositories) =>
      this.edgeRepository(repositories).all(),
    )
  }

  getEdgesByType(type: EdgeType): OmniEdge[] {
    return this.attempt(`get edges by type ${type}`, [], (repositories) =>
      this.edgeRepository(repositories).byType(type),
    )
  }

  getOutEdges(nodeId: string): OmniEdge[] {
    return this.attempt(`get outgoing edges ${nodeId}`, [], (repositories) =>
      this.edgeRepository(repositories).outgoing(nodeId),
    )
  }

  getInEdges(nodeId: string): OmniEdge[] {
    return this.attempt(`get incoming edges ${nodeId}`, [], (repositories) =>
      this.edgeRepository(repositories).incoming(nodeId),
    )
  }
  ...
  getMeta(key: string): string | null {
    return this.attempt(`get metadata ${key}`, null, (repositories) =>
      repositories.graph.getMeta(key),
    )
  }
  ...
  loadGraph(): OmniGraph {
    return this.attempt('load graph', { nodes: [], edges: [] }, (repositories) =>
      repositories.graph.load(),
    )
  }
  ...
  removeDanglingEdges(): number {
    return this.attempt('remove dangling edges', 0, (repositories) =>
      this.edgeRepository(repositories).removeDangling(),
    )
  }
  ...
  getNodesByTypes(types: NodeType[]): OmniNode[] {
    return this.attempt('get nodes by types', [], (repositories) =>
      repositories.nodes.byTypes(types),
    )
  }

  getDownstreamNodes(nodeId: string, edgeTypes?: EdgeType[]): OmniNode[] {
    return this.attempt('get downstream nodes', [], (repositories) =>
      repositories.graph.downstream(nodeId, edgeTypes),
    )
  }

  getUpstreamNodes(nodeId: string, edgeTypes?: EdgeType[]): OmniNode[] {
    return this.attempt('get upstream nodes', [], (repositories) =>
      repositories.graph.upstream(nodeId, edgeTypes),
    )
  }

  findNodeByRoute(route: string): OmniNode | null {
    return this.attempt('find node by route', null, (repositories) =>
      repositories.nodes.findByRoute(route),
    )
  }

  findNodeByFilePath(filePath: string): OmniNode | null {
    return this.attempt('find node by file', null, (repositories) =>
      repositories.nodes.findByFilePath(filePath),
    )
  }

  findNodeByAny(query: string): OmniNode | null {
    return (
      this.findNodeByRoute(query) ??
      this.findNodeByFilePath(query) ??
      this.attempt('find node by name', null, (repositories) =>
        repositories.nodes.findByName(query),
      )
    )
  }

  getAffectedPages(nodeId: string, maxDepth = 10): OmniNode[] {
    return this.attempt('get affected pages', [], (repositories) =>
      repositories.graph.affectedPages(nodeId, maxDepth),
    )
  }

  getSubtree(rootId: string, edgeType: EdgeType, maxDepth: number): GraphSubtree | null {
    return this.attempt('get subtree', null, (repositories) =>
      repositories.graph.subtree(rootId, edgeType, maxDepth),
    )
  }
  ...
  private edgeRepository(repositories: Repositories): EdgeRepository {
    if (!(repositories.edges instanceof EdgeRepository))
      throw new Error('Edge repository is unavailable')
