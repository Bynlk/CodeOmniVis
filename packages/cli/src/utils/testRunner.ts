...
export function createTestRunPlan(request: TestRunRequest): TestRunPlan {
  ...
  switch (request.runner) {
    case 'vitest': args = ['exec', 'vitest', '--run', ...request.extraArgs]; break
    case 'jest': args = ['exec', 'jest', '--runInBand', ...request.extraArgs]; break
    case 'playwright': args = ['exec', 'playwright', 'test', ...request.extraArgs]; break
    case 'cypress': args = ['exec', 'cypress', 'run', ...request.extraArgs]; break
    case 'gradle': {
      command = path.join(root, process.platform === 'win32' ? 'gradlew.bat' : 'gradlew')
      if (!fs.existsSync(command)) throw new Error('Validated Gradle wrapper was not found in the project root')
      args = ['test', ...request.extraArgs]
      break
    }
    default: throw new Error(`Unsupported test runner: ${String(request.runner)}`)
  }
...
export async function runTestRunner(request: TestRunRequest): Promise<TestRunResult> {
  ...
  return new Promise(resolve => {
    const child = spawn(plan.command, plan.args, { ...plan.options, stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    const finish = (exitCode: number | null, signal: NodeJS.Signals | null): void => {
      ...
      resolve({ exitCode, signal, timedOut, stdout: stdout.toString('utf8'), stderr: stderr.toString('utf8'), truncated })
    }
