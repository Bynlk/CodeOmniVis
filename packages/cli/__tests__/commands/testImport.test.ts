...
describe('test-import command', () => {
  ...
  it('appends JUnit provenance through the transactional snapshot store', async () => {
    ...
    fs.writeFileSync(path.join(root, 'app', 'page.tsx'), 'export default function Home() { return <main /> }')
    fs.writeFileSync(path.join(root, 'junit.xml'), '<testsuite name="Home"><testcase name="renders" time="0.01" /></testsuite>')
  ...
  it('rejects input outside the project before replacing the snapshot', async () => {
    ...
    await expect(runTestImport({ project: root, junit: path.join(outside, 'junit.xml') })).rejects.toThrow('inside the project')
    const db = new OmniDatabase(getDbPath(root))
    await db.ready()
