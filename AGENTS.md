# AGENTS.md — CodeOmniVis 项目 Codex 工作指南

> 本文件是 Codex 在本项目中工作的唯一权威指南。
...
| Skill | 适用场景 |
|-------|---------|
| `.Codex/skills/parser-development.md` | 编写解析器时参考（模板、分工、测试） |
| `.Codex/skills/cytoscape-visualization.md` | 实现 UI 可视化时参考（Cytoscape.js、React 封装） |
| `.Codex/skills/mcp-server.md` | 实现 MCP Server 时参考（工具模板、返回格式） |
| `.Codex/skills/sqlite-storage.md` | 实现存储层时参考（Schema、批量插入、查询） |
| `.Codex/skills/monorepo-patterns.md` | 管理 monorepo 时参考（pnpm、Turborepo、包依赖） |
| `.Codex/skills/testing-patterns.md` | 编写测试时参考（Vitest、fixture、mock） |
| `.Codex/skills/error-handling.md` | 处理错误时参考（降级原则、错误级别） |
