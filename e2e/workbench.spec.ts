...
test('navigates the workbench and degrades on a graph API failure', async ({ page }) => {
  ...
  await expect(page.getByRole('application', { name: 'Graph visualization canvas' })).toBeVisible()
  ...
  await page.getByRole('textbox', { name: 'Search nodes' }).fill('BookingList')
  ...
  await page.reload()
  await expect(page.getByTestId('canvas-error')).toContainText(
